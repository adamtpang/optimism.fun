...
type SortKey =
  ...
  | 'capital'
  | 'voices'
type ColKey = 'humans' | 'bcr' | 'itn' | 'util' | 'time' | 'cap' | 'co' | 'v'

const ALL_TIERS: Tier[] = ['welfare', 'x-risk', 'hard-tech', 'progress', 'emerging']
const ALL_COLS: ColKey[] = ['humans', 'bcr', 'itn', 'util', 'time', 'cap', 'co', 'v']
    ...
    decorated.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'utility':
          ...
          break
        case 'time':
          // Lower time-to-impact ranks higher under "desc", so invert.
                ...
                >
                  ...
                  {colShown('time') && (
                    <td className="px-3 py-3 text-right tabular-nums text-ink-300">
                      {p.timeToImpact ? (
                        formatYears(p.timeToImpact.value)
                      ) : (
                        <span className="text-ink-500/70">·</span>
                      )}
                    </td>
                  ...
                  {colShown('cap') && (
                    <td className="px-3 py-3 text-right tabular-nums text-ink-300">
                      {p.capitalRequired ? (
                        formatUSD(p.capitalRequired.value)
                      ) : (
                        <span className="text-ink-500/70">·</span>
                      )}
                    </td>
